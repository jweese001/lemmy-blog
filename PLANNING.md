...
└─────────────────────────────────────────────────────────────────────┘
  ...
  ┌──────────┐     ┌──────────┐     ┌──────────┐     ┌──────────┐
  │  TOPIC   │────▶│ RESEARCH │────▶│  WRITE   │────▶│  IMAGE   │
  │ SELECTOR │     │  (Web)   │     │   (MD)   │     │(FlowBoard)│
  └──────────┘     └──────────┘     └──────────┘     └──────────┘
                                                            │
  ┌──────────┐     ┌──────────┐     ┌──────────┐            │
  │  SOCIAL  │◀────│  DEPLOY  │◀────│   GIT    │◀───────────┘
...
|-----------|------------|--------|
| Static Site | Hugo | ✅ Complete |
| Theme | Custom "w33s3" | ⚠️ Needs styling |
| Hosting | Netlify | ✅ Deployed |
| Version Control | GitHub (private) | ✅ Connected |
| Image Generation | FlowBoard | ✅ Available |
| FlowBoard Integration | Python/Playwright | ❌ Not started |
| Automation | Cron + Script | ❌ Not started |
| Social Posting | Bluesky/Mastodon | ❌ Not started |
...
async def generate_post(topic: str):
    """Full pipeline: research → write → image → deploy"""
...
|----------|-----|------|--------|
| Bluesky | AT Protocol | App password | ❌ Not started |
| Mastodon | REST API | OAuth | ❌ Not started |
| RSS | Built-in | None | ✅ Hugo generates |
...
│   ├── generate_post.py
│   ├── flowboard_integration.py
│   └── social_posting.py
└── workflows/                   # FlowBoard workflow JSONs
    └── *.json
```
