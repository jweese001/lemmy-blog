...
def ensure_bluesky_cta(content: str) -> str:
    """Insert the standard Bluesky CTA once, after the article body and before sources/footer."""
    stripped = content.rstrip()
    without_profile_line = re.sub(
        rf"(?m)^.*{re.escape(BLUESKY_PROFILE_URL)}.*\n?",
        "",
        stripped,
    ).rstrip()

    if SOURCES_BLOCK_MARKER in without_profile_line:
        body, sources_block = without_profile_line.split(SOURCES_BLOCK_MARKER, 1)
        return body.rstrip() + "\n\n" + BLUESKY_CTA + "\n" + SOURCES_BLOCK_MARKER + sources_block

    if FLOWBOARD_FOOTER_MARKER in without_profile_line:
        body, footer = without_profile_line.split(FLOWBOARD_FOOTER_MARKER, 1)
        return body.rstrip() + "\n\n" + BLUESKY_CTA + "\n" + FLOWBOARD_FOOTER_MARKER + footer

    return (without_profile_line + "\n\n" if without_profile_line else "") + BLUESKY_CTA
...
"""
    ...
    full_content = frontmatter + "\n" + ensure_bluesky_cta(content) + footer
