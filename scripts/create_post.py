...
def ensure_bluesky_cta(content: str) -> str:
    ...
    if "https://bsky.app/profile/lemmysmic.bsky.social" in stripped:
        normalized = stripped.replace(BLUESKY_CTA + SOURCES_BLOCK_MARKER, BLUESKY_CTA + "\n" + SOURCES_BLOCK_MARKER)
        normalized = normalized.replace(BLUESKY_CTA + FLOWBOARD_FOOTER_MARKER, BLUESKY_CTA + "\n" + FLOWBOARD_FOOTER_MARKER)
        return normalized

    if SOURCES_BLOCK_MARKER in stripped:
        body, sources_block = stripped.split(SOURCES_BLOCK_MARKER, 1)
        return body.rstrip() + "\n\n" + BLUESKY_CTA + "\n" + SOURCES_BLOCK_MARKER + sources_block

    if FLOWBOARD_FOOTER_MARKER in stripped:
        body, footer = stripped.split(FLOWBOARD_FOOTER_MARKER, 1)
